...
without <- function(table,col,list, # this list here works opposite to the list in the function above. here we identify what we don't want.
  ...
  orphanids <- dbGetQuery(conn,
                          paste0(
                            "SELECT COUNT(DISTINCT(PATID)) FROM ",schema,".",prefix,toupper(table)," WHERE PATID NOT IN (SELECT DISTINCT(PATID) FROM ",schema,".",prefix,"DEMOGRAPHIC WHERE BIRTH_DATE > '",ref_date1,"' AND BIRTH_DATE  < '",ref_date2,"')"
                          ))
  ...
  pats_wit_oneout <- dbGetQuery(conn,
                                paste0("SELECT COUNT(DISTINCT(PATID)) FROM ",schema,".",prefix,toupper(table)," WHERE ",toupper(col), " IS NULL OR CAST(",toupper(col), " AS CHAR(54)) IN  ('",paste(list,collapse = "','"),"')")
  )
  ...
  whatsoever <- dbGetQuery(conn,
                                paste0("SELECT COUNT(DISTINCT(PATID)) FROM ",toupper(table)," WHERE PATID NOT IN (SELECT DISTINCT(PATID) FROM ",schema,".",prefix,toupper(table)," WHERE ",toupper(col), " IS NOT NULL AND CAST(",toupper(col), " AS CHAR(54)) NOT IN  ('",paste(list,collapse = "','"),"'))")
  )
  #calculate the percentage
...
orphankeys <- function(table1, #source table
  ...
  orphans <- dbGetQuery(conn,
                          paste0(
                            "SELECT COUNT(DISTINCT(",toupper(col),")) FROM ",schema,".",prefix,toupper(table2)," WHERE ",toupper(col)," NOT IN (SELECT DISTINCT(",toupper(col),") FROM ",schema,".",prefix,toupper(table1),")"
                          ))
